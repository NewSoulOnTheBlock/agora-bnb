...
///      TORII is a *tax* token, so the amount that lands in the pair is smaller
...
/**
 * @title ToriiVault
 * @notice TORII's Flap tax vault on BNB Chain — the contract Flap pays the 5%
 ...
 *   - after DEX migration, tax is taken **in TORII itself**.
 *
 * TORII is therefore not quote revenue and is deliberately excluded from the
 * baseline. Treating it as revenue would be worse than useless: the Treasury
 * marks TORII at zero when computing NAV, so forwarding the token leg raw would
...
contract ToriiVault is VaultBaseV3, ReentrancyGuard {
    ...
    /// @notice Cumulative TORII sold for BNB.
    ...
    modifier onlyGuardian() {
        require(msg.sender == _getGuardian(), "ToriiVault: not guardian");
        _;
    }

    constructor(address treasury_, address taxToken_, address quoteToken_, address router_) {
        require(treasury_ != address(0), "ToriiVault: zero treasury");
        require(taxToken_ != address(0), "ToriiVault: zero tax token");
        require(router_ != address(0), "ToriiVault: zero router");
        ...
        require(quoteToken_ == address(0), "ToriiVault: quote must be native");
     ...
     * revert. Reverting here would make Flap's dispatch of TORII fail.
     */
    ...
    function forwardQuote() external nonReentrant returns (uint256 amount) {
        amount = _forwardAll();
        require(amount > 0, "ToriiVault: nothing to forward");
    }

    /**
     * @notice Sell the TORII tax leg for BNB and forward the proceeds.
     * @param amountIn     TORII to sell; `0` means the entire balance.
    ...
    {
        require(deadline >= block.timestamp, "ToriiVault: deadline passed");
        require(minQuoteOut > 0, "ToriiVault: minQuoteOut required");

        uint256 balance = taxToken.balanceOf(address(this));
        uint256 sellAmount = amountIn == 0 ? balance : amountIn;
        require(sellAmount > 0, "ToriiVault: no tokens to convert");
        require(sellAmount <= balance, "ToriiVault: amount exceeds balance");

        uint256 quoteBefore = address(this).balance;

        // forceApprove: TORII may be a non-standard ERC20, and a stale non-zero
        ...
        require(received >= minQuoteOut, "ToriiVault: insufficient output");

        cumulativeConverted += sellAmount;
        emit Converted(sellAmount, received);

        forwarded = _forwardAll();
        require(forwarded > 0, "ToriiVault: nothing to forward");
    }
    ...
    /// @notice TORII held and awaiting conversion — the post-graduation tax leg.
    ...
    /// @notice Guardian escape hatch for any stuck ERC-20, including TORII.
    ...
    function description() public pure override returns (string memory) {
        return
            "TORII reserve vault. The 5% trade tax is forwarded to the TORII Treasury, "
    ...
    function vaultUISchema() public pure override returns (VaultUISchema memory schema) {
        schema.vaultType = "ToriiReserveVault";
        schema.description =
            "Routes TORII's trade tax into the TORII Treasury. Anyone may call the "
        ...
        schema.methods[1].description = "TORII held by the vault, awaiting conversion to BNB.";
        schema.methods[1].inputs = new FieldDescriptor[](0);
        schema.methods[1].outputs = new FieldDescriptor[](1);
        schema.methods[1].outputs[0] = FieldDescriptor("amount", "uint256", "TORII awaiting conversion", 18);
        ...
        schema.methods[3].description =
            "Forward all recognised BNB revenue to the TORII Treasury. Anyone may call this.";
        ...
        schema.methods[4].description =
            "Sell the TORII tax leg for BNB on PancakeSwap and forward the proceeds to the "
            "Treasury. Set minQuoteOut to bound slippage. Anyone may call this.";
        schema.methods[4].inputs = new FieldDescriptor[](3);
        schema.methods[4].inputs[0] = FieldDescriptor("amountIn", "uint256", "TORII to sell; 0 sells the full balance", 18);
