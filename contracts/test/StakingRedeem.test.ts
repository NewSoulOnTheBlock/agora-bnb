...
describe("StakedTorii + Redeemer", () => {
  let owner: HardhatEthersSigner;
  let alice: HardhatEthersSigner;
  let bob: HardhatEthersSigner;
  let carol: HardhatEthersSigner;

  let torii: any, treasury: any, feeSink: any, escrow: any, staking: any, redeemer: any;
  ...
  beforeEach(async () => {
    ...
    await torii.transfer(alice.address, 100_000_000n * WAD); // 10%
    await torii.transfer(bob.address, 100_000_000n * WAD); // 10%
  });

  // =========================================================================
  describe("StakedTorii — ERC-4626 shape", () => {
    it("names itself off the underlying", async () => {
      expect(await staking.name()).to.equal("Staked Torii");
      expect(await staking.symbol()).to.equal("stTORII");
      expect(await staking.asset()).to.equal(await torii.getAddress());
    });

    it("mints shares 1:1 because ETH rewards stay out of totalAssets", async () => {
      const amt = 1000n * WAD;
      await torii.connect(alice).approve(await staking.getAddress(), amt);
    ...
    it("does NOT inflate share price when rewards arrive", async () => {
      ...
      // Share price must be unchanged — the vault holds no extra TORII.
    ...
    it("lets stakers withdraw their principal", async () => {
      ...
      expect(await torii.balanceOf(alice.address)).to.equal(100_000_000n * WAD);
    });
  });

  // =========================================================================
  describe("StakedTorii — reward accounting", () => {
    beforeEach(async () => {
      for (const who of [alice, bob]) {
        await torii.connect(who).approve(await staking.getAddress(), 1000n * WAD);
      }
    });
  ...
  describe("Redeemer — the floor mechanism", () => {
    ...
    it("burns on request, so supply drops immediately", async () => {
      await torii.connect(alice).approve(await redeemer.getAddress(), stake);
      ...
      expect(await torii.balanceOf(alice.address)).to.equal(0n);
    });
    ...
    it("never overpays: redeeming the WHOLE supply cannot drain more than NAV", async () => {
      ...
      const all = await torii.balanceOf(owner.address);
      await torii.transfer(alice.address, all);
      await torii.connect(bob).transfer(alice.address, await torii.balanceOf(bob.address));
      const total = await torii.balanceOf(alice.address);

      await torii.connect(alice).approve(await redeemer.getAddress(), total);
    ...
    it("pays min(snapshot, current) when NAV falls between request and execute", async () => {
      ...
      await torii.connect(bob).approve(await redeemer.getAddress(), stake);
    ...
    it("refuses to redeem when there is no floor yet", async () => {
      ...
      await bare.setAgora(await torii.getAddress());
      const r2 = await (
        await ethers.getContractFactory("Redeemer")
      ).deploy(await torii.getAddress(), await bare.getAddress(), owner.address);

      await torii.connect(alice).approve(await r2.getAddress(), stake);
  ...
  describe("end to end: tax in, yield out, floor up", () => {
    it("runs the whole economic loop", async () => {
      ...
      await torii.connect(bob).approve(await redeemer.getAddress(), 50_000_000n * WAD);
