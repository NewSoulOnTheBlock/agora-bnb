...
async function main() {
  ...
  line();
  const torii = await (
    await ethers.getContractFactory("MockTorii")
  ).deploy(1_000_000_000n * 10n ** 18n);
  await torii.waitForDeployment();
  ...
  console.log(`  token            ${await torii.getAddress()}`);
  ...
  line();
  await (await treasury.setAgora(await torii.getAddress())).wait();
